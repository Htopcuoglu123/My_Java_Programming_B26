...
*/
    ...
    the cardholder name, and the balance. The card number has to be 16 characters long to be a valid card number.
    ...
    The card type must be either MasterCard or Visa to be valid card type.
     If it is not one of those two then do not store the type and print “invalid card type”.
      ...
      If the pin is more than or less than 4 characters then do not store the pin into the
  ...
  */
    public DebitCard(long cardNumber,String holderName,double balance){
        ...
        if(String.valueOf(cardNumber).length()==16){//we wanted to know the length of the number thats why we converted it to a String
            //using the valueOf method.We could have also done (""+cardNumber).length()
            this.cardNumber=cardNumber;
        }else{
            System.out.println("Invalid card number");//System.err.println "err" makes the output red
        }
    }
